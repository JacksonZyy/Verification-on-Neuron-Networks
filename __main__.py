...
parser.add_argument('--left_bottom_cornor', type=int, nargs=2, default=None, help='the left botton cornor of the rectangular region, x axis and y axis')
parser.add_argument('--right_up_cornor', type=int, nargs=2, default= None, help='the right up cornor of the rectangular region, x axis and y axis')
############ End of my parser
...
if(config.sig_beta!=0):
	assert args.left_bottom_cornor!= None and args.right_up_cornor != None, "two points of the region must be provided"
	assert args.left_bottom_cornor[0] >=0 and args.left_bottom_cornor[1] >=0, "The coordinates of the left_bottom_cornor CANNOT be less than 0"
	x_min = args.left_bottom_cornor[0]
	x_max = args.right_up_cornor[0]
	y_min = args.left_bottom_cornor[1]
	y_max = args.right_up_cornor[1]
	if(dataset=='mnist'):
		assert x_max <=27 and y_max <=27, "The coordinates of the right_up_cornor CANNOT be greater than the height and width of the MNIST image"
		...
		assert x_max <=31 and y_max <=31, "The coordinates of the right_up_cornor CANNOT be greater than the height and width of the CIFAR10 image"
        ...
        if(label == int(test[0])): #test[0] is the ground truth lable
            ...
            if(config.sig_beta!=0):
	            if(dataset=='mnist'):
	            	...
	            	my_specLB, my_specUB = image
	            	my_specLB[sig_indexes] = np.clip(image - my_beta,0,1)[sig_indexes]
	            	my_specLB[small_indexes] = np.clip(image - my_epsilon,0,1)[small_indexes]
	            	my_specUB[sig_indexes] = np.clip(image + my_beta,0,1)[sig_indexes]
	        		my_specUB[small_indexes] = np.clip(image + my_epsilon,0,1)[small_indexes]
	            elif(dataset=='cifar10'):
	            	my_specLB, my_specUB = image
	            	for index in sig_indexes:
	            		count=3*index
	            		my_specLB[count] = max(min(1, image[count]-my_beta), 0)
	            		my_specUB[count] = max(min(1, image[count]+my_beta), 0)
	            		count = count + 1
	            		my_specLB[count] = max(min(1, image[count]-my_beta), 0)
	            		my_specUB[count] = max(min(1, image[count]+my_beta), 0)
	            		count = count + 1
	            	...
	            	for index in small_indexes:
	            		count=3*index
	            		my_specLB[count] = max(min(1, image[count]-my_epsilon), 0)
	            		my_specUB[count] = max(min(1, image[count]+my_epsilon), 0)
	            		count = count + 1
	            		my_specLB[count] = max(min(1, image[count]-my_epsilon), 0)
	            		my_specUB[count] = max(min(1, image[count]+my_epsilon), 0)
	            		count = count + 1
